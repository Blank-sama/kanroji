...
from ShikimoriRobot import  dispatcher , DEV_USERS
...
from ShikimoriRobot.helper_extra.admin_rights import user_can_changeinfo, user_can_promote
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton(
            "Demote", callback_data="demote_shikimori({})".format(user_member.user.id))
    ]])     
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton(
              "Promote", callback_data="promote_shikimori({})".format(user_member.user.id))
       ]]) 
...
def refresh_admin(update, _):
    ...
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton(
            "admin cache", callback_data="admin_shikimori({})".format(user_member.user.id))
    ]])
                    ...
                    [
                        [
                            InlineKeyboardButton(
                             ...
                             InlineKeyboardButton(
                                "Unpin", callback_data="pin_shikimori")                           
                        ]
                    ]
                ), 
                parse_mode=ParseMode.HTML,
...
def button(update: Update, context: CallbackContext) -> str:
    ...
    match = re.match(r"demote_shikimori\((.+?)\)", query.data)
    if match:
...
def promote_button(update: Update, context: CallbackContext) -> str:
    ...
    user: Optional[User] = update.effective_user
    bot: Optional[Bot] = context.bot
    match = re.match(r"promote_shikimori\((.+?)\)", query.data)
    if match:
        user_id = match.group(1)
        chat: Optional[Chat] = update.effective_chat
        member = chat.get_member(user_id)
...
DEMOTE_SHIKIMORI_HANDLER = CallbackQueryHandler(button, pattern=r"demote_shikimori", run_async=True)
PROMOTE_SHIKIMORI_HANDLER = CallbackQueryHandler(promote_button , pattern=r"promote_shikimori", run_async = True)
...
dispatcher.add_handler(DEMOTE_SHIKIMORI_HANDLER)
dispatcher.add_handler(PROMOTE_SHIKIMORI_HANDLER)
...
__handlers__ = [
    ...
    SET_STICKER_HANDLER,
    SETCHATPIC_HANDLER,
    DEMOTE_SHIKIMORI_HANDLER,
    RMCHATPIC_HANDLER,
    SETCHAT_TITLE_HANDLER,
    ADMINLIST_HANDLER,
    PROMOTE_SHIKIMORI_HANDLER,
    PIN_HANDLER,
    UNPIN_HANDLER,
    PINNED_HANDLER,
    INVITE_HANDLER,
